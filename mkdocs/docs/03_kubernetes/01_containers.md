...
In Linux this environment is called a container. On Windows there are no native container concepts and therefore we do not further mention this OS.
...
`Go to the directory where your static binary is (e.g. /myapps/app1/)`
```shell
cd /myapps/app1
```
`Create/write the Dockerfile (e.g. with vim)`
```shell
vim Dockerfile
```
`Dockerfile`
